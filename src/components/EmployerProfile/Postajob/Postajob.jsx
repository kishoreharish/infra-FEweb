...
import { TextField, Select, MenuItem, Button, Box, Typography, Input } from "@mui/material";
...
const PostAJob = () => {
  const [formData, setFormData] = useState({
    ...
    required_skills: "",
    company_logo: null,
    created_date: new Date().toISOString().split('T')[0],  // Default to today's date
    expire_date: "",
  ...
  const handleChange = (e) => {
    ...
    if (name === "company_logo") {
      ...
      // Automatically set status to "Inactive" if expiry date is in the past
  ...
  const handleSubmit = async () => {
    ...
    // Validation check
    if (!formData.title || !formData.company || !formData.salary || !formData.location || !formData.description || !formData.skills || !formData.deadline || !formData.company_logo) {
  ...
  return (
    <div className={styles.container}>
      ...
      <Box className={styles.form}>
        ...
        <Box className={styles.row}>
          <Select name="job_type" value={formData.job_type} onChange={handleChange} displayEmpty fullWidth>
        ...
        <TextField label="Required Fields" name="required_skills" value={formData.required_skills} onChange={handleChange} fullWidth required />

        <Box className={styles.row}>
          <TextField label="Created Date" name="created_date" value={formData.created_date} disabled fullWidth />
        ...
        <Box className={styles.row}>
          ...
          <Input type="file" name="company_logo" onChange={handleChange} fullWidth required />
        </Box>
